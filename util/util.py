...
# def draw_result_pic(dir, epoch, train, val, title, label1, label2):
#     fig1 = plt.figure(figsize=(7,4))
#     # epo = np.arange(epoch+1)
#     epo = np.arange(len(train))
   
#     os.makedirs(os.path.dirname(dir), exist_ok=True)
#     a2 = fig1.add_subplot(1, 1, 1)
#     a2.plot(epo, train, epo, val)
#     a2.set_title(title)
#     a2.legend(labels=[label1, label2])
#     a2.set_ylabel(title)
...
def draw_similarity_pic(dir, epoch, positive, negative):
    
    epo = np.arange(len(positive))
    ...
    # Positive Similarity
    axs[0].plot(epo, positive, color='tab:blue', linewidth=1.5)
    axs[0].set_title("Positive Mean Angular Difference")
    axs[0].set_ylabel("Degrees")
    axs[0].grid(axis='y', linestyle='dashed', alpha=0.7)

    # Negative Similarity
    axs[1].plot(epo, negative, color='tab:orange', linewidth=1.5)
    axs[1].set_title("Negative Mean Angular Difference")
