...
def find_max_point(maximum_distance, num_spk, data, ):
    ...
    for j in range(num_spk):
        ...
        near_degree=np.arange(-maximum_distance, maximum_distance+1)+current_max.item()
...
#         azimuth=azimuth_B.repeat_interleave(len(calc_layer), 0)


#         for frame_num in range(output_B.shape[-1]):

#             vad_frame=vad_B[..., frame_num] # B, num_spk
...
#             if active_spk_num==3:
#                 print('3 speakers are active in the frame, skip this frame')

#             out_frame=output_B[..., frame_num] # B, 360//resolution
...
#             if estimate_frame.ndim!=2:
#                 print('Invalid estimate_frame dimension')
...
#             acc=acc.astype(int).mean(axis=1)
#             #acc = acc.astype(int).mean()
#             doa_error=doa_error.sum(axis=-1)

#             if np.isnan(acc).any():
#                 print('NaN occurred in acc')
